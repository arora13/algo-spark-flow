...
const Dashboard = () => {
  ...
  const completionRate = Math.round(
    ((user.progress.completedAlgorithms.length + user.progress.solvedProblems.length) /
      (totalAlgorithms + totalProblems)) *
      100
  );
  ...
  const achievements = [
    {
      ...
      earned: user.progress.completedAlgorithms.length > 0,
    },
    {
      name: 'Problem Solver',
      description: 'Solved 5 coding problems',
      earned: user.progress.solvedProblems.length >= 5,
    },
    {
      name: 'Algorithm Master',
      description: 'Completed 5 algorithms',
      earned: user.progress.completedAlgorithms.length >= 5,
    },
    {
      name: 'Dedicated Learner',
      description: 'Earned 500 points',
      earned: user.progress.totalScore >= 500,
    },
  ];
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {user.progress.completedAlgorithms.length}/{totalAlgorithms}
                  </p>
                </div>
                <span className="text-2xl">📖</span>
              </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {user.progress.solvedProblems.length}/{totalProblems}
                  </p>
                </div>
                <span className="text-2xl">💻</span>
              </div>
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Progress
                    value={(user.progress.completedAlgorithms.length / totalAlgorithms) * 100}
                    className="h-2"
                  />
                </div>
                ...
                <div>
                  ...
                  <Progress
                    value={(user.progress.solvedProblems.length / totalProblems) * 100}
                    className="h-2"
                  />
                </div>
